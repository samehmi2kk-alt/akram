...
# تهيئة اتصال Firebase باستخدام ملف الاعتمادات الجديد
...
def home():
    ...
    for doc in invoices_ref:
        ...
        inv_data['id'] = doc.id
        # تحويل تاريخ التاريخ لو مخزن كـ string أو timestamp
    ...
    # حساب التكلفة والأرباح التقديرية
    total_cost = 0
    for inv in invoices:
...
def create_invoice():
    ...
    # خصم المخزون وتجهيز تفاصيل المنتجات داخل الفاتورة
    processed_items = []
    for item in items:
    ...
    update_time, inv_ref = db.collection('invoices').add(invoice_data)
